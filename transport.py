...
class TransportManager:
    ...
    def register_student_direct(self, student_id, student_name, route_number, route_name, driver_name, driver_phone, pickup_time):
        ...
        if route_number <= 0:
            return False, "Please enter a valid route number."

        # Driver Phone Validation
        if not driver_phone.isdigit() or len(driver_phone) != 11:
            return False, "Please enter a valid driver phone number. (Must be 11 digits)"
    ...
    def process_request(self):
        ...
        while True:
            driver_phone = input("Enter Driver Phone: ")
            if driver_phone.isdigit() and len(driver_phone) == 11:
                break
            print("Invalid phone number! Must be 11 digits. Please try again.")
    ...
    def update_record(self):
        ...
        if choice == 1:
            try:
                new_route_num = int(input("Enter new Route Number: "))
                if new_route_num <= 0:
                    print("Invalid Route Number! Update failed.")
                    return
                found.route_number = new_route_num
            except ValueError:
                print("Invalid Route Number!")
                return
            ...
            new_driver_phone = input("Enter new Driver Phone: ")
            if not new_driver_phone.isdigit() or len(new_driver_phone) != 11:
                print("Invalid phone number! Must be 11 digits. Update failed.")
                return
            found.driver_phone = new_driver_phone
        elif choice == 3:
