...
#### 1. Singly Linked List
  **Used In**: `StudentManager` (`student.py`) and `TransportManager` (`transport.py`).   **Concept**: A collection of nodes where each node contains data and a reference (pointer) to the next node.
  **Why used here?**:
