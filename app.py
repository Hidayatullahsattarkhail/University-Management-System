...
def index():
    return render_template('index.html', 
                           ...
                           fee_count=fee_mgr.count)

# ... (Student, Exam, Transport routes remain unchanged) ...

# --- Fee Routes ---
@app.route('/fees')
def fees():
    return render_template('fees.html', fees=fee_mgr.records)

@app.route('/fees/add', methods=['POST'])
def add_fee():
    try:
        student_id = int(request.form['student_id'])
        amount = request.form['amount'] # Keep as string for validation in manager
        
        success, message = fee_mgr.submit_fee(student_id, amount)
        if success:
            fee_mgr.save_to_file()
...
def add_exam():
    try:
        ...
        if not student_mgr.search_by_id(student_id):
            flash("Error: Student ID not found!", 'error')
