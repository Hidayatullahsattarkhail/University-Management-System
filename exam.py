...
class ExamManager:
    ...
    def create_exam_record(self, student_id, student_name, course_code, course_name, marks):
        ...
        if any(char.isdigit() for char in student_name):
             return False, "Invalid student name. Please enter alphabetic characters only."
    ...
    def add_exam_record(self, student_mgr=None):
        print("\n--- Add Exam Record ---")
        try:
        ...
        if student_mgr:
            student = student_mgr.search_by_id(student_id)
            if not student:
                print("Error: Student ID not found! Please add the student first.")
                return
    ...
    def menu(self, student_mgr=None):
        while True:
            ...
            if choice == 1:
                self.add_exam_record(student_mgr)
            elif choice == 2:
                self.display_all()
            elif choice == 3:
                self.search_record()
