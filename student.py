...
class StudentManager:
    ...
    def create_student(self, student_id, name, department, phone, email):
        ...
        if self.search_by_id(student_id):
            return False, "This Student ID already exists. Please enter a unique ID."

        # 2. Name Validation
        # Allow spaces in name but check for digits
        if any(char.isdigit() for char in name):
             return False, "Invalid name. Please enter alphabetic characters only."
        
        # 3. Department Selection Validation
        valid_departments = ['Computer Science', 'Electrical Engineering', 'Business Administration', 'Mathematics', 'Physics']
        if department not in valid_departments:
            return False, f"Please select a valid department. Valid options: {', '.join(valid_departments)}"

        # 4. Phone Number Validation
        # Check if it's 11 digits and numeric
        if not phone.isdigit() or len(phone) != 11:
            return False, "Please enter a valid phone number. (Must be 11 digits)"

        # 5. Email Validation
        if '@' not in email or '.' not in email.split('@')[-1] or len(email.split('@')[0]) == 0:
            return False, "Please enter a complete and valid email address."
    ...
    def update_student(self):
        ...
        if choice == 1:
            print(f"Valid Departments: {', '.join(['Computer Science', 'Electrical Engineering', 'Business Administration', 'Mathematics', 'Physics'])}")
            new_dept = input("Enter new Department: ")
            if new_dept not in ['Computer Science', 'Electrical Engineering', 'Business Administration', 'Mathematics', 'Physics']:
                print("Invalid department! Update failed.")
                return
            found.department = new_dept
        elif choice == 2:
            new_phone = input("Enter new Phone Number: ")
            if not new_phone.isdigit() or len(new_phone) != 11:
                print("Invalid phone number! Must be 11 digits. Update failed.")
                return
            found.phone_number = new_phone
        elif choice == 3:
            new_email = input("Enter new Email: ")
            if '@' not in new_email or '.' not in new_email.split('@')[-1] or len(new_email.split('@')[0]) == 0:
                print("Invalid email address! Update failed.")
                return
